...
struct dim
{
    ...
    constexpr dim& operator*=(int val) noexcept
    {
        cx *= val;
        cy *= val;
        return *this;
    ...
    constexpr dim& operator*=(dim val) noexcept
    {
        cx *= val.cx;
        cy *= val.cy;
        return *this;
...
constexpr bool operator==(const dim& lhs, const dim& rhs) noexcept
{
...
constexpr dim operator*(dim d0, dim d1) noexcept
{
    return d0 *= d1;
}
constexpr dim operator/(dim d0, dim d1) noexcept
...
constexpr dim operator*(dim d, int val) noexcept
{
    return d *= val;
}
constexpr dim operator/(dim d, int val) noexcept
...
constexpr dim operator*(int val, dim d) noexcept
{
    return d *= val;
}
...
struct rect
{
    ...
    constexpr rect(point pos, dim size) noexcept
        : left(pos.x), top(pos.y), right(size.cx), bottom(size.cy)
    {
    }
    ...
    img::dim dimensions() const noexcept
    {
        return img::dim { abs(right - left), abs(bottom - top) };
    }

    constexpr img::point get_top_left() const noexcept
    {
        return img::point { left, top };
    }
    constexpr img::point get_bottom_right() const noexcept
    {
        return img::point { right, bottom };
    }
};
...
constexpr bool is_normalized(rect r) noexcept
{
...
constexpr bool is_empty_rect(rect r) noexcept
{
...
{
    return !(region.bottom < 0 || region.top > image_dim.cy || region.right < 0
             || region.left > image_dim.cx);
}
...
/**
     ...
     */
constexpr img::rect intersect_with_image_dim(const img::rect tmp_region,
                                             img::dim image_dim) noexcept
{
