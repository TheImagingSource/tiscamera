...
#elif defined __ARM_NEON || defined __ARM_NEON__ || defined __ARM_ARCH_7__ \
...
enum cpu_features
{
    ...
    CPU_C = 0x0001,
    CPU_ARM_A7 =
        0x0002, // supported by ARM Cortex-A7 processors (raspberry pi 2 + 3) (Note: this is not AARCH32)
    CPU_ARM_A8 = 0x0004, // supported by ARM Cortex-A8 processors, this is AARCH32
    CPU_ARM_A64 =
        0x0008, // supported by ARM Cortex-A8+ processors 64-bit OS, this also known as AARCH64
...
enum cpu_features
{
    ...
    // skip 0x0080
    CPU_AVX512_F =
        0x0100, // expands most 32-bit and 64-bit based AVX instructions with EVEX coding scheme to support 512-bit registers, operation masks, parameter broadcasting, and embedded rounding and exception control
    CPU_AVX512_CD =
        0x0200, // Conflict Detection Instructions (CDI) – efficient conflict detection to allow more loops to be vectorized
    CPU_AVX512_BW =
        0x0400, // Byte and Word Instructions (BW) – extends AVX-512 to cover 8-bit and 16-bit integer operations
    CPU_AVX512_DQ =
        0x0800, // Doubleword and Quadword Instructions (DQ) – adds new 32-bit and 64-bit AVX-512 instructions
    CPU_AVX512_VL =
        0x1000, // Vector Length Extensions (VL) – extends most AVX-512 operations to also operate on XMM (128-bit) and YMM (256-bit) registers

    CPU_AVX512_IFMA =
        0x2000, // AVX-512 Integer Fused Multiply Add (IFMA) - fused multiply add of integers using 52-bit precision.
    CPU_AVX512_VBMI =
        0x4000, // AVX-512 Vector Byte Manipulation Instructions (VBMI) adds vector byte permutation instructions which were not present in AVX-512BW.
    CPU_AVX512_VPOPCNTDQ =
        0x8000, // Vector population count instruction. Introduced with Knights Mill and Ice Lake.[7]

    CPU_AVX512_VNNI =
        0x0001'0000, // AVX-512 Vector Neural Network Instructions (VNNI) - vector instructions for deep learning.
    CPU_AVX512_VBMI2 =
        0x0002'0000, // AVX-512 Vector Byte Manipulation Instructions 2 (VBMI2) - byte/word load, store and concatenation with shift.
    CPU_AVX512_BITALG =
        0x0004'0000, // AVX-512 Bit Algorithms (BITALG) - byte/word bit manipulation instructions expanding VPOPCNTDQ.

    CPU_AVX512_GFNI = 0x0008'0000, // EVEX-encoded Galois field new instructions:
    CPU_AVX512_VPCLMULQDQ =
        0x0010'0000, // VPCLMULQDQ with AVX-512F adds EVEX-encoded 512-bit version of PCLMULQDQ instruction
    CPU_AVX512_VAES = 0x0020'0000, // EVEX-encoded AES instructions.

    CPU_AVX512_VP2INTERSECT =
        0x0040'0000, // AVX-512 Vector Pair Intersection to a Pair of Mask Registers (VP2INTERSECT).
    ...
    CPU_UsesSSSE3 =
        CPU_C | CPU_SSSE3, // this is currently our minimum of support for SIMD operations
    ...
    CPU_UsesAVX512_BASE0 =
        CPU_UsesAVX512_F | CPU_AVX512_CD | CPU_AVX512_VL | CPU_AVX512_DQ | CPU_AVX512_BW,
    ...
    CPU_COREi_TIGERLAKE =
        CPU_UsesAVX512_BASE0 | CPU_AVX512_IFMA | CPU_AVX512_VBMI, // 10xxGx   ~2019
