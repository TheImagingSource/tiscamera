...
struct color_matrix
{
    union
    {
        struct
        {
            int16_t r_rfac, r_gfac, r_bfac, g_rfac, g_gfac, g_bfac, b_rfac, b_gfac, b_bfac;
        };
        int16_t fac[9];
    };

    static constexpr color_matrix get_defaults() noexcept
    {
        return get_increased_saturation();
    }
    static constexpr color_matrix get_increased_saturation() noexcept
    {
        return color_matrix { 90, -13, -13, -13, 90, -13, -13, -13, 90 };
    }
    static constexpr color_matrix get_neutral() noexcept
    {
        return color_matrix { 64, 0, 0, 0, 64, 0, 0, 0, 64 };
    }

    constexpr int16_t& operator[](int idx) noexcept
    {
        return fac[idx];
    }
    constexpr int16_t operator[](int idx) const noexcept
    {
        return fac[idx];
    }
};
...
struct tonemapping_params
{
    ...
    static constexpr tonemapping_params get_defaults() noexcept
    {
        return tonemapping_params {};
    }
};
