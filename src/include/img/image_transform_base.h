...
struct img_descriptor
{
    ...
    int data_length =
        0; // the actual data length valid in this buffer. this is in most cases calc_img_size( type, dim_x, dim_y ), but may be smaller for e.g. compressed formats
    ...
    {
        flags_no_wrap_beg =
            0x1, // it is not necessary to wrap on the start line (e.g. debayering may access p_data + pitch * -1 and p_data + pitch * -2)
        flags_no_wrap_end =
            0x2, // it is not necessary to wrap on the end line (e.g. debayering may access p_data + data_length + pitch and p_data + data_length + pitch
        flags_no_flip =
            0x4, // this indicates that the image is already flipped, this flag is needed when separating the image in strips
    };

    constexpr fourcc fourcc_type() const noexcept
    {
        return static_cast<fourcc>(type);
    }

    constexpr uint8_t* data() const noexcept
    {
        return static_cast<uint8_t*>(data_.planes[0].plane_ptr);
    }

    /** Returns the image stride in bytes. Note: This only works for the first plane */
    constexpr int pitch() const noexcept
    {
        return data_.planes[0].pitch;
    }
    ...
    constexpr img::dim dimensions() const noexcept
    {
        return dim;
    }
    constexpr bool empty() const noexcept
    {
        return dim.empty() || pitch() == 0 || type == 0 || data() == nullptr;
    }
    constexpr img::img_type to_img_type() const noexcept
    {
        return make_img_type(type, dim);
    }
};
...
constexpr img_descriptor make_img_desc_raw(img::img_type type,
                                           img_planar_layout_data plane0) noexcept;
constexpr img_descriptor make_img_desc_raw(fourcc fcc,
                                           img::dim dim,
                                           int data_length,
                                           img_plane plane0,
...
constexpr img_descriptor make_img_desc_raw(fourcc fcc,
                                           img::dim dim,
                                           int data_length,
                                           img_planar_layout_data plane0,
                                           uint32_t flags = 0) noexcept;
constexpr img_descriptor make_img_desc_from_linear_memory(img::img_type in_type,
                                                          void* data_ptr) noexcept;

constexpr img_descriptor replace_fcc(const img::img_descriptor& desc, uint32_t new_fcc) noexcept;
...
constexpr uint8_t* get_line_start_of_plane(const img::img_descriptor& dsc,
                                           int y,
                                           int plane_index) noexcept;

template<class TOut> constexpr TOut* get_line_start(const img::img_descriptor& dsc, int y) noexcept;
template<class TOut>
constexpr TOut* get_line_start_of_plane(const img::img_descriptor& dsc,
                                        int y,
                                        int plane_index) noexcept;
...
constexpr img_descriptor make_img_desc_from_linear_memory(img::img_type in_type,
                                                          void* data_ptr) noexcept
{
    if (img::is_multi_plane_format(in_type.type))
    {
        img_planar_layout_data plane_data;
        auto* data_ptr_iter = static_cast<uint8_t*>(data_ptr);
        for (int plane_index = 0; plane_index < img::planar::get_plane_count(in_type.fourcc_type());
             ++plane_index)
        {
        ...
        return make_img_desc_raw(
            in_type.fourcc_type(), in_type.dim, in_type.buffer_length, plane_data);
    }
    const int pitch = img::calc_minimum_pitch(in_type);
    return make_img_desc_raw(
        in_type.fourcc_type(), in_type.dim, in_type.buffer_length, img_plane { data_ptr, pitch });
}
                                           ...
                                           uint32_t flags /*= 0*/) noexcept
{
    return img_descriptor {
        static_cast<uint32_t>(fcc), dim, data_length, flags, { plane0 },
    };
}
constexpr img_descriptor make_img_desc_raw(fourcc fcc,
                                           img::dim dim,
                                           int data_length,
                                           img_planar_layout_data layout,
                                           uint32_t flags /*= 0*/) noexcept
{
    return img_descriptor {
        static_cast<uint32_t>(fcc), dim, data_length, flags, layout,
    };
}

constexpr img_descriptor replace_fcc(const img::img_descriptor& dsc, uint32_t new_fcc) noexcept
{
    auto res = dsc;
    res.type = new_fcc;
    return res;
...
constexpr img_descriptor flip_image_in_img_desc(const img_descriptor& dsc) noexcept
{
    ...
    for (int index = 0; index < img::calc_planar_fmt_count_of_planes(dsc.type); ++index)
    {
...
constexpr uint8_t* get_line_start_of_plane(const img::img_descriptor& dsc,
                                           ...
                                           int plane_index) noexcept
{
    return static_cast<uint8_t*>(dsc.data_.planes[plane_index].plane_ptr)
           + y * dsc.data_.planes[plane_index].pitch;
}

inline img::img_descriptor extract_plane(const img::img_descriptor& dsc,
                                         int plane_index,
                                         fourcc fcc) noexcept
{
    const auto plane = dsc.data_.planes[plane_index];
    ...
    const img::dim actual_dim { static_cast<int>(dsc.dim.cx * info.scale_dim_x),
                                static_cast<int>(dsc.dim.cy * info.scale_dim_y) };
...
{
    const auto info = planar::get_fcc_info(dsc.fourcc_type(), plane_index);
    return extract_plane(dsc, plane_index, info.equiv_fcc);
}
} // namespace img
