...
#include <img/image_transform_data_structs.h>      // img::color_matrix
...
struct property_cont_gain
{
    ...
    float gain_db_multiplier =
        6.0f; // According to Tim the db mul factor for some sensors is 6.f (and not 3.01f as previously implemented)
};
...
struct auto_focus_params
{
    ...
    int device_focus_val =
        0; // the currently set focus value, this should be the actual value, that is present in the device

    bool is_end_cmd = false; // when set, the auto_focus code stops the current auto run
    bool is_run_cmd =
        false; // when set, the auto_focus code resets and takes the run_cmd_params to init itself
    struct run_cmd_param_struct
    {
        ...
        int focus_device_speed =
            500; // Legacy and GigECam = 500 (will most likely never change ...)
        int auto_step_divisor =
            4; // Legacy cam = 4, GigECam this is read from  IInteger 'FocusAutoStepDivisor' otherwise 4
        bool suggest_sweep =
            false; // Legacy cam = false, GigECam this is read from IInteger 'FocusAutoSweepHint' otherwise false
    } run_cmd_params;
};
