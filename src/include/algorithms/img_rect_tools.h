...
/**
     ...
     */
img::img_descriptor make_safe_img_view(const img::img_descriptor& data,
                                       const img::rect& r) noexcept;
...
/** Clips the input RECT to the actual img_desc region.
     ...
     */
img::rect clip_to_img_desc_region(img::rect roi,
                                  img::point partial_scan_offset,
                                  img::dim pixel_dim,
                                  img::dim image_dim) noexcept;
img::rect clip_to_img_desc_region(img::rect roi,
                                  img::point partial_scan_offset,
                                  img::dim pixel_dim,
                                  const img::img_descriptor& image) noexcept;

inline img::rect clip_to_img_desc_region(img::rect roi,
                                         img::point partial_scan_offset,
                                         img::dim pixel_dim,
                                         const img::img_descriptor& image) noexcept
{
...
constexpr bool is_full_image(const img::rect& r, const img::dim image_dim) noexcept
{
    return (r.left == 0 && r.top == 0)
           && ((r.bottom == 0 && r.right == 0)
               || (r.bottom == image_dim.cy && r.right == image_dim.cx));
}
constexpr bool is_full_image(const img::rect& r, const img::img_descriptor& data) noexcept
{
