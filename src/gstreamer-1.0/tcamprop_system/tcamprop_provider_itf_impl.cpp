...
void fwd_error_message(const report_error& report_err_func,
                       tcamprop_system::error_id id,
                       std::string_view dsc = {})
{
    if (report_err_func)
    {
...
void fwd_error_message(const report_error& report_err_func,
                       tcamprop_system::error_id id,
                       const tcamprop_system::property_info& info,
                       std::string_view dsc)
{
    if (report_err_func)
    {
        report_err_func(id,
                        fmt::format("Property '{}' ({}), {}.",
                                    info.prop_name,
                                    tcamprop_system::to_string(info.type),
                                    dsc));
    }
}
void fwd_error_message(const report_error& report_err_func,
                       ...
                       std::error_code errc)
{
    ...
    {
        report_err_func(id,
                        fmt::format("Property '{}' ({}), {}. Error-message: '{}'",
                                    info.prop_name,
                                    tcamprop_system::to_string(info.type),
                                    dsc,
                                    errc.message()));
...
GSList* tcamprop_system::tcamprop_impl_get_tcam_property_names(
    property_list_interface* prop_list_itf,
    const report_error& report_err_func)
{
    ...
    for (const auto& prop_name : prop_list_itf->get_property_list())
    {
        ...
        {
            fwd_error_message(report_err_func,
                              error_id::property_failed_get_flags,
                              prop,
                              "failed to get flags",
                              prop_flags_opt.error());
            continue;
        }
...
gboolean tcamprop_system::tcamprop_impl_get_tcam_property(property_list_interface* prop_list_itf,
                                                          const gchar* name,
                                                          GValue* value,
                                                          GValue* min,
                                                          GValue* max,
                                                          ...
                                                          const report_error& report_err_func)
{
    ...
    {
        fwd_error_message(report_err_func,
                          error_id::property_not_found,
                          fmt::format("Failed to find property '{}'.", name));
        return FALSE;
    }
    ...
    else if (prop.type == tcamprop_system::prop_type::boolean)
    {
        if (value)
        {
            ...
            {
                fwd_error_message(report_err_func,
                                  error_id::property_failed_get_value,
                                  prop,
                                  "failed to get value",
                                  tmp.error());
                return FALSE;
        ...
        if (def)
        {
            ...
            {
                fwd_error_message(report_err_func,
                                  error_id::property_failed_get_range,
                                  prop,
                                  "failed to get range",
                                  range_opt.error());
                return FALSE;
    ...
    {
        fwd_error_message(report_err_func,
                          error_id::property_type_incompatible,
                          prop,
...
GSList* tcamprop_system::tcamprop_impl_get_menu_entries(property_list_interface* prop_list_itf,
                                                        const gchar* name,
                                                        const report_error& report_err_func)
{
    if (prop_list_itf == nullptr || name == nullptr)
    ...
    if (desc.type != tcamprop_system::prop_type::menu)
    {
        fwd_error_message(report_err_func,
                          error_id::property_type_incompatible,
                          desc,
                          fmt::format("expected type ({})", tcamprop_system::prop_type::menu));
        return nullptr;
    }
    auto range_opt = prop_itf->get_property_range();
    if (!range_opt)
    {
        fwd_error_message(report_err_func,
                          error_id::property_type_incompatible,
                          desc,
                          "failed to fetch menu-items due to",
                          range_opt.error());
        return nullptr;
    }

    GSList* ret = nullptr;
    for (const auto& m : range_opt.value().menu_entries)
    {
        ret = g_slist_append(ret, g_strdup_string_view(m));
    }
    return ret;
}
                                                          ...
                                                          const report_error& report_err_func)
{
    ...
    switch (prop.type)
    {
        case tcamprop_system::prop_type::button:
        {
            ...
            {
                fwd_error_message(report_err_func,
                                  error_id::set_function_failed,
                                  prop,
                                  fmt::format("err='{}'", err.message()));
                return FALSE;
            }
            return TRUE;
        }
        case tcamprop_system::prop_type::boolean:
        {
            if (!G_VALUE_HOLDS_BOOLEAN(value))
            {
                fwd_error_message(
                    ...
                    error_id::property_type_incompatible,
                    prop,
                    fmt::format("expected boolean, got {}", g_type_name(g_value_get_gtype(value))));
                return FALSE;
            }
        ...
        case tcamprop_system::prop_type::integer:
        {
            if (!G_VALUE_HOLDS_INT(value))
            {
                fwd_error_message(
                    ...
                    error_id::property_type_incompatible,
                    prop,
                    fmt::format("expected integer, got {}", g_type_name(g_value_get_gtype(value))));
                return FALSE;
            }
        ...
        case tcamprop_system::prop_type::real:
        {
            if (!G_VALUE_HOLDS_DOUBLE(value))
            {
                fwd_error_message(
                    ...
                    error_id::property_type_incompatible,
                    prop,
                    fmt::format("expected real, got {}", g_type_name(g_value_get_gtype(value))));
                return FALSE;
            }
        ...
        case tcamprop_system::prop_type::menu:
        {
            ...
            if (G_VALUE_HOLDS_STRING(value))
            {
                ...
                if (it == menu_entries.end())
                {
                    fwd_error_message(report_err_func,
                                      error_id::property_type_incompatible,
                                      prop,
                                      fmt::format("failed to find menu entry for '{}'", str));
                    return FALSE;
                }
            ...
            else if (G_VALUE_HOLDS_INT(value))
            {
                ...
                {
                    fwd_error_message(report_err_func,
                                      error_id::parameter_invalid_value,
                                      prop,
                                      fmt::format("failed to find menu entry for '{}'", index));
                    return FALSE;
                }
            }
            else
            {
                fwd_error_message(report_err_func,
                                  error_id::property_type_incompatible,
                                  prop,
                                  fmt::format("expected int or string, got {}",
                                              g_type_name(g_value_get_gtype(value))));
                return FALSE;
            }
