...
namespace
{
template<typename TObj> struct delete_gobject_helper
{
    void operator()(TObj* ptr) noexcept
    {
        unref_object(ptr);
    }
};
...
template<typename T> using gst_unique_ptr = std::unique_ptr<T, delete_gobject_helper<T>>;
...
inline gst_unique_ptr<GstPad> get_static_pad(const gst_unique_ptr<GstElement>& elem,
                                             std::string name)
{
