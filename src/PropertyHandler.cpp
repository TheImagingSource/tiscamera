                                                 ...
                                                 std::shared_ptr<PropertyImpl> impl)
{
    ...
    if (type == TCAM_PROPERTY_TYPE_BOOLEAN)
    {
        return std::make_shared<Property>(
            PropertyBoolean(impl, p->get_struct(), p->get_value_type()));
    }
    else if (type == TCAM_PROPERTY_TYPE_INTEGER)
    {
        return std::make_shared<Property>(
            PropertyInteger(impl, p->get_struct(), p->get_value_type()));
    }
    else if (type == TCAM_PROPERTY_TYPE_DOUBLE)
    {
        return std::make_shared<Property>(
            PropertyDouble(impl, p->get_struct(), p->get_value_type()));
    }
    else if (type == TCAM_PROPERTY_TYPE_STRING)
    {
        return std::make_shared<Property>(
            PropertyString(impl, p->get_struct(), p->get_value_type()));
    }
    else if (type == TCAM_PROPERTY_TYPE_ENUMERATION)
    {
        ...
        return std::make_shared<Property>(
            PropertyEnumeration(impl, p->get_struct(), s.get_mapping(), p->get_value_type()));
    }
    else if (type == TCAM_PROPERTY_TYPE_BUTTON)
    {
        return std::make_shared<Property>(
            PropertyButton(impl, p->get_struct(), p->get_value_type()));
    }
    return nullptr;
}
...
void PropertyHandler::generate_properties()
{
    ...
    for (auto& p : external_properties) { handle_flags(p); }
...
void PropertyHandler::handle_flags(std::shared_ptr<Property>& p)
{
    ...
    switch (p->get_ID())
    {
        ...
        {
            std::shared_ptr<Property> pea =
                find_property(device_properties, TCAM_PROPERTY_EXPOSURE_AUTO);
            ;
        ...
        {
            auto pea = find_property(device_properties, TCAM_PROPERTY_GAIN_AUTO);
