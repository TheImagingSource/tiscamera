...
#include "transform/fcc1x_packed_to_fcc8_internal.h"
#include "transform/transform_pwl_functions.h"
...
static void	auto_sample_by16img( const img::img_descriptor& image, auto_sample_points& points )
{
    ...
    {
        uint8_t v00 = cur_line[(x + 0) * 2];    // this should read the high byte for a by16 entry
        uint8_t v01 = cur_line[(x + 1) * 2];
        uint8_t v10 = nxt_line[(x + 0) * 2];
        uint8_t v11 = nxt_line[(x + 1) * 2];
...
void auto_alg::impl::auto_sample_by_imgu8( const img::img_descriptor& image, auto_sample_points& points )
{
    ...
    else
    {
        ...
        if( auto pack_type = get_fcc1x_pack_type( image.fourcc_type() ); pack_type != fccXX_pack_type::invalid )
        {
            ...
            case fccXX_pack_type::fcc10_mipi:       auto_sample_bayer_image( image, points, &by10or12_to_sample_entries<calc_fcc10_packed_mipi_to_fcc8> ); return;
            ...
            case fccXX_pack_type::fcc12_mipi:       auto_sample_bayer_image( image, points, &by10or12_to_sample_entries<calc_fcc12_packed_mipi_to_fcc8> ); return;
...
auto_alg::impl::resulting_brightness	auto_alg::impl::auto_sample_mono_imgu8( const img::img_descriptor& image )
{
    ...
    {
        return auto_sample_mono_img_( image, []( const void* line, int x ) -> uint8_t { return reinterpret_cast<const uint16_t*>(line)[x]; } );
    }
    ...
    else if( img::is_mono_fcc( image.fourcc_type() ) )
    {
        ...
        case fccXX_pack_type::fcc10_mipi:           return auto_sample_mono_img_( image, &calc_fcc10_packed_mipi_to_fcc8 );
        ...
        case fccXX_pack_type::fcc12_mipi:           return auto_sample_mono_img_( image, &calc_fcc12_packed_mipi_to_fcc8 );
