...
void debayer_ccm_rgb24_nn(unicap_data_buffer_t* destbuf,
                          ...
                          debayer_data_t* data)
{
    ...
    for (j = 1; j < height - 1; j += 2)
    {
        ...
        for (i = 0; i < width - 1;)
        {
            ...
            *dest++ = ((unsigned int)source[lineoffset + i + width]
                       + (unsigned int)source[lineoffset + i + 1])
                      / 2;
            ...
            *dest++ =
                ((unsigned int)source[lineoffset + i + width + 1] + (int)source[lineoffset + i])
                / 2;
        ...
        for (i = 0; i < width - 1;)
        {
            ...
            *dest++ =
                ((unsigned int)source[lineoffset + i + width] + (int)source[lineoffset + i + 1])
                / 2;
...
void debayer_ccm_rgb24_nn_be(unicap_data_buffer_t* destbuf,
                             unicap_data_buffer_t* srcbuf,
                             debayer_data_t* data)
{
    int i, j;
...
void debayer_ccm_rgb24_gr_nn(unicap_data_buffer_t* destbuf,
                             unicap_data_buffer_t* srcbuf,
                             debayer_data_t* data)
{
    int i, j;
...
void debayer_ccm_uyvy(unicap_data_buffer_t* destbuf,
                      unicap_data_buffer_t* srcbuf,
                      debayer_data_t* data)
{
    int dest_offset = 0;
...
static void gst_bayer2rgb_split_and_upsample_horiz(guint8* dest0,
                                                   guint8* dest1,
                                                   const guint8* src,
                                                   int n)
{
    ...
    bayer_orc_horiz_upsample_unaligned(dest0 + 2, dest1 + 2, src + 1, (n - 4) >> 1);
#else
...
typedef void (*process_func)(guint8* d0,
                             const guint8* s0,
                             const guint8* s1,
                             const guint8* s2,
                             const guint8* s3,
...
static void gst_bayer2rgb_process(GstBayer2RGB* bayer2rgb,
                                  uint8_t* dest,
                                  int dest_stride,
                                  uint8_t* src,
                                  int src_stride)
...
#define LINE(x) (tmp + ((x)&7) * bayer2rgb->width)
    ...
    gst_bayer2rgb_split_and_upsample_horiz(
        LINE(3 * 2 + 0), LINE(3 * 2 + 1), src + 1 * src_stride, bayer2rgb->width);
    j = 0;
    gst_bayer2rgb_split_and_upsample_horiz(
        LINE(j * 2 + 0), LINE(j * 2 + 1), src + j * src_stride, bayer2rgb->width);
