...
{
	return ((TDataType*)(((byte*)region_base) + y * stride)) + x;
}
...
{
	if( image.type == FOURCC_Y16 || img::isBayer16FCC( image.type ) )
	{
...
#if defined KERNEL_DRIVER_
	RegionInfo* regions = new(NonPagedPool, 'UCOF') RegionInfo[region_count];
#else
...
img::auto_focus::auto_focus()
	: focus_applied_( 1 ), img_wait_cnt( 0 ), focus_min_( 0 ), focus_max_( 0 ),
	max_time_to_wait_for_focus_change_( 1000 )//, min_time_to_wait_for_focus_change_( 1000 )
{
...
void img::auto_focus::run( int focus_val, int min, int max, const RECT& roi, int speed, int auto_step_divisor, bool suggest_sweep )
{
	focus_min_ = min;
	focus_max_ = max;
...
bool img::auto_focus::auto_alg_run( const img::img_descriptor& img, const auto_alg::auto_focus_params& state, POINT offsets, SIZE pixel_dim, int& new_focus_val )
{
    // these formats should work ...
    if( img.type != FOURCC_Y800 &&
        img.type != FOURCC_Y16 &&
        isBayer16FCC( img.type ) &&
        isBayerFCC( img.type ) &&
    ...
    if( state.is_run_cmd )
    {
        ...
        rval = analyze_frame( img, offsets, pixel_dim, new_focus_val );
    }
    else if( state.is_end_cmd )
    {
        end();
    }
    else
...
bool	img::auto_focus::analyze_frame( const img::img_descriptor& img, POINT offsets, SIZE pixel_dim, int& new_focus_val )
{
	...
	{
		if( img.dim_x != init_width_ || img.dim_y != init_height_ || img.pitch != init_pitch_ || init_pixel_dim_.cx != pixel_dim.cx || init_pixel_dim_.cy != pixel_dim.cy ||
...
bool	img::auto_focus::analyze_frame_( const img::img_descriptor& img, int& new_focus_val )
{
	data.stepCount += 1;
...
unsigned int img::auto_focus::get_sharpness( const img::img_descriptor& img )
{
	RegionInfo	info;
	info.x = data.x;
	info.y = data.y;
...
bool img::auto_focus::check_wait_condition()
{
	...
	{
#if defined KERNEL_DRIVER_
		return dutil::system_time::time::now() > img_wait_endtime;
#else
...
#if defined KERNEL_DRIVER_
	img_wait_endtime = dutil::system_time::time::now( ) + dutil::system_time::time::time( ms_to_use, dutil::MilliSecond );
#else
...
void img::auto_focus::find_region( const img::img_descriptor& image, RECT roi, RegionInfo& region )
{
