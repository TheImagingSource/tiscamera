...
static unsigned int calc_exposure(float dist,
                                  int exposure,
                                  const auto_alg::property_cont_exposure& range)
{
    dist = (dist + 2.f) / 3;
    ...
    const auto_alg::property_cont_iris& iris_desc)
{
    ...
    if (exposure_desc.auto_enabled && gain_desc.auto_enabled && gain > gain_desc.min
        && exposure < exposure_desc.max // we can reduce gain, because we can increase exposure
    )
    {
        rval.exposure = CLIP((exposure * 105) / 100,
                             exposure_desc.min,
                             exposure_desc.max); // increase exposure by 5%
        return rval;
    }

    if (exposure_desc.auto_enabled && iris_desc.auto_enabled && iris > iris_desc.min
        && exposure < exposure_desc.max // we can reduce iris, because we can increase exposure
    )
    {
        rval.exposure = CLIP((exposure * 105) / 100,
                             exposure_desc.min,
    ...
    if (gain_desc.auto_enabled && iris_desc.auto_enabled && gain > gain_desc.min
        && iris < iris_desc.max // we can reduce gain, because we can increase iris
    )
    {
...
int auto_alg::impl::calc_auto_pwm_iris(float corrected_brightness,
                                       int reference_value,
                                       const auto_alg::property_cont_iris& iris_desc,
