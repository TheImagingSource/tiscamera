...
static int calc_iris(int dist, int iris, const algorithms::property_cont& range)
{
    //if( dist >= dist_mid )    // when we have to reduce, we reduce it faster
...
algorithms::gain_exposure_iris_values algorithms::calc_auto_gain_exposure_iris(
    int brightness,
    int reference_value,
    ...
    const algorithms::property_cont& iris_desc)
{
    ...
    if (exposure_desc.do_auto && gain_desc.do_auto && gain > gain_desc.min
    ...
    if (exposure_desc.do_auto && iris_desc.do_auto && iris > iris_desc.min
    ...
    if (gain_desc.do_auto && iris_desc.do_auto && gain > gain_desc.min
