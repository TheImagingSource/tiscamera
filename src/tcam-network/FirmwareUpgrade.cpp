...
static bool parseManufacturerSpecificInformation(const std::string& info,
                                                 int& typeId,
...
bool getDeviceInfo(const Packet::ACK_DISCOVERY& discoveryResponse,
                   int& typeId,
                   std::string& modelName)
{
...
bool isPackageFile(const std::string& fileName)
{
    ...
    {
        return (0
                == fileName.compare(fileName.length() - ending.length(), ending.length(), ending));
    }
    else
    {
        return false;
...
std::vector<byte> extractFileFromPackage(const std::string& packageFileName,
...
Status uploadBlackfinFirmware(IFirmwareWriter& dev, std::vector<unsigned char>& data)
{
    ...
    dev.write(
        0xEF000004, (1 << 2), 3000 /* longer timeout, eeprom may take a while */); // erase app
...
Status upgradeBlackfinFirmware(IFirmwareWriter& dev,
                               const std::string& fileName,
...
Status findFirmwareInPackage(const std::string& fileName,
...
Status uploadAndVerify(IFirmwareWriter& dev,
...
Status uploadGigEFPGAFirmware(IFirmwareWriter& dev,
...
Status uploadFPGAConfiguration(IFirmwareWriter& dev,
...
Status upgradeFPGAFirmwareFromPackage(IFirmwareWriter& dev,
                                      const std::string& fileName,
                                      const std::string& modelName,
                                      std::function<void(int, const std::string&)> progressFunc)
{
    ...
    std::string fpgaConfigurationName;
    unsigned int requiredFPGAVersion;
    Status status = findFirmwareInPackage(
        fileName, modelName, firmwareName, fpgaConfigurationName, requiredFPGAVersion);
    if (failed(status))
    {
        return status;
    }
...
Status upgradeFPGAFirmwareDirect(IFirmwareWriter& dev,
                                 const std::string& fileName,
                                 std::function<void(int, const std::string&)> progressFunc)
{
...
Status upgradeFPGAFirmware(IFirmwareWriter& dev,
                           const std::string& fileName,
                           const std::string& modelName,
                           std::function<void(int, const std::string&)> progressFunc)
{
