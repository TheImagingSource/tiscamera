...
static void dfk73_close(struct dfk73_devinfo* devinfo)
{
    ...
    if (devinfo->driver_detached)
    {
        ...
        for (itf = 0; itf < 1; itf++) { libusb_attach_kernel_driver(devinfo->dev, itf); }
    }
    free(devinfo);
}


static int dfk73_xu_register_read(struct dfk73_devinfo* devinfo, uint16_t addr)
{
    int ret;
...
void dfk73_prepare_all(void)
{
    ...
    for (usbdev = dfk73_find_device(idx++); usbdev; usbdev = dfk73_find_device(idx++))
    {
...
int dfk73_v4l2_prepare(char* devfile)
{
    ...
    // Map the EM27595Register control
    unsigned char uvcctl_data[] = {
        0x20, 0x09, 0x98, 0x00, 0x45, 0x4d, 0x32, 0x37, 0x35, 0x39, 0x35, 0x52, 0x65, 0x67,
        0x69, 0x73, 0x74, 0x65, 0x72, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x52, 0xf2, 0xb8, 0xaa, 0xd1, 0x8e,
        0x72, 0x49, 0x8c, 0xed, 0x96, 0xb1, 0x7f, 0x04, 0x40, 0x8b, 0x02, 0x20, 0x00, 0x00,
