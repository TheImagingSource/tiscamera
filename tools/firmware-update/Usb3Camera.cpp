                                     ...
                                     std::function<void(int)> progress)
{
    ...
    for (unsigned int addr = 0; addr < firmware.size(); addr += BLOCK_SIZE)
    {
        ...
        if (ret < 0)
        {
            throw std::runtime_error("Unable to write eeprom. Libusb returned "
                                     + std::to_string(ret) + ".");
        }
        else
        {
...
#define MAX_WRITE_SIZE \
    2048 //(2 * 1024) // Max. size of data that can be written through one vendor command.
...
bool Usb3Camera::initialize_eeprom(std::vector<uint8_t>& firmware)
{
    ...
    while (index < filesize)
    {
        ...
        if (length != 0)
        {
            for (int i = 0; i < length; i++) { checksum += data_p[2 + i]; }
