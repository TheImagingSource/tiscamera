...
private:
    ...
    /// @param progress - callback function
    /// @return integer response from libusb; >= 0 on success
    int upload_firmware_file(unsigned char* buffer,
                             unsigned int size,
                             std::function<void(int)> progress);
    ...
    /// @param strserial
    /// @return integer response from libusb
    int write_strings(const char* strvendor, const char* strproduct, const char* strserial);

    void get_strings(char* strvendor, char* strproduct, char* strserial, int bufsize);
