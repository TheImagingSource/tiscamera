...
namespace gencp
{
GenCPDevice::GenCPDevice(std::shared_ptr<driver_interface::IUsbDevice> dev) : dev_ { dev } {}
...
void GenCPDevice::read_mem(uint64_t address, uint8_t* buffer, uint16_t length) const
{
    ...
    dev_->write_vendor_request(
        VendorCommands::GENCP, 0, 0, cmd.data(), static_cast<uint16_t>(cmd.size()));

    std::vector<uint8_t> ack_buffer(device_read_buffer_size());
    dev_->read_vendor_request(
        VendorCommands::GENCP, 0, 0, ack_buffer.data(), static_cast<uint16_t>(ack_buffer.size()));
