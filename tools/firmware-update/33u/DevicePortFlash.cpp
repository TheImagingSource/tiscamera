...
void DevicPortFlash::upload(Camera& dev,
...
void DevicPortFlash::upload_internal(Camera& cam,
                                     const std::vector<UploadItem>& items,
                                     util::progress::IReportProgress& progress)
{
    ...
    auto erase_items_progress =
        util::progress::MapItemProgress(erase_progress, static_cast<int>(erase_requests.size()));
    ...
    auto items_items_progress =
        util::progress::MapItemProgress(items_progress, static_cast<int>(items.size()));

    for (auto item : items)
    {
        cam.flash().write_verify(item.offset,
                                 item.data.data(),
                                 static_cast<uint32_t>(item.data.size()),
                                 items_items_progress);
